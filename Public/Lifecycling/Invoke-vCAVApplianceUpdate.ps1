function Invoke-vCAVApplianceUpdate(){
    ...
    $UpdateStatusTask = (Invoke-vCAVAPIRequest -URI $URI -Method Get ).JSONData
    if((Watch-TaskCompleted -Task $UpdateStatusTask -Timeout ((Get-PowerCLIConfiguration -Scope Session).WebOperationTimeoutSeconds))){
        ...
        if($CheckOnly){
            ...
            if(!$AcceptEULA){
                ...
                $UpdateEULATask = (Invoke-vCAVAPIRequest -URI $EULAURI -Method Get ).JSONData
            ...
            $UpdateApplianceTask = (Invoke-vCAVAPIRequest -URI $URI -Method Post ).JSONData
